...
namespace {
class ClientMethod final : public LuaCpp::LuaMetaObject {
 public:
  explicit ClientMethod(miet::lambda::http::ClientPtr httpClient)
  ...
  int Execute(LuaCpp::Engine::LuaState& L) override {
    ...
    auto request = miet::lambda::http::Request::Default();
  ...
  std::optional<int> PopulateAdditionalParams(
      LuaCpp::Engine::LuaState& L, miet::lambda::http::Request& request) {
    const auto query =
  ...
  void BuildLuaResponse(LuaCpp::Engine::LuaState& L,
                        const miet::lambda::http::Response& response) {
    lua_newtable(L);
...
Client::Client(miet::lambda::http::ClientPtr httpClient) noexcept
    : httpClient_(std::move(httpClient)) {
  getClientMethod_ = std::make_shared<ClientMethod>(httpClient_);
}

std::shared_ptr<LuaCpp::Engine::LuaType> Client::getValue(std::string& key) {
  if (key == "send") {
